...
ColorGradingEffect::ColorGradingEffect(const std::string& name)
{
	name_ = name;
...
void ColorGradingEffect::Update()
{
...
{
	dxManager_->GetCommandList()->SetPipelineState(psoManager_->GetOffScreenPSO(OffScreenEffectType::kColorGrading));
...
void ColorGradingEffect::CreateEffectResource()
{
	auto* resourceManager = dxManager_->GetResourceManager();
	effectHandle_ = resourceManager->CreateUploadBuffer(sizeof(ColorGradingData), L"ColorGradingEffect");
	effectDataPtr_ = reinterpret_cast<ColorGradingData*>(resourceManager->Map(effectHandle_));
	*effectDataPtr_ = effectData_;
}
