...
struct BoneAttackParams {
	...
	// 予備動作の間、プレイヤーへ向き直る速さの上限[度/秒]。0 なら敵の普段の速さのまま。
	// 溜めの間ずっと予兆がぴったりプレイヤーを追うと、避ける手段が回避の無敵時間しか無くなる。
	// 遅くしておくと、横へ走り込めば予兆の外へ出られる（向きは判定が出る瞬間に固定される）
	float windupTurnSpeed = 0.0f;
