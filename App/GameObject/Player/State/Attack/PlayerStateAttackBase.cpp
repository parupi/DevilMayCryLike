...
PlayerStateAttackBase::PlayerStateAttackBase(std::string attackName)
{
	...
	// 移動系
	gv->AddItem(name_, "MoveSpeed", float());          // 攻撃中にどれくらい移動するか
	gv->AddItem(name_, "KnockBackSpeed", float());     // 敵のノックバック
...
void PlayerStateAttackBase::UpdateAttackData()
{
	...
	// 移動系
	attackData_.moveSpeed = gv->GetInstance()->GetValueRef<float>(name_, "MoveSpeed");
	attackData_.knockBackSpeed = gv->GetInstance()->GetValueRef<float>(name_, "KnockBackSpeed");
